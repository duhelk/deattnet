...
'''
Significant parts of the code are based on 
...
from deattnet.training_tools.train_epoch import train_epoch
from deattnet.training_tools.val import val
from deattnet.training_tools.get_clusters import get_clusters
from deattnet.training_tools.train_util import save_checkpoint
from deattnet.datasets.data_util import input_transform, input_transform_1d
from deattnet.datasets.cambridge import CambridgeLandmarks
from deattnet.datasets.msls import MSLS
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='DeAttNet:Depth Attention Net')

    parser.add_argument('--config_path', type=str, default=join(ROOT_DIR, 'configs/train.ini'),
    ...
    parser.add_argument('--save_path', type=str, default=join(ROOT_DIR, 'out'),
    ...
    parser.add_argument('--dataset_root_dir', type=str, default='/media/HD2/Workspace/datasets/MSLS',
                        help='Root directory of dataset')
                        #Cambridge Landmarks
    parser.add_argument('--identifier', type=str, default='cambridge_vgg16_rgb',
                        help='Description of this model, e.g. mapillary_nopanos_vgg16_netvlad')
    parser.add_argument('--nEpochs', type=int, default=30, help='number of epochs to train for')
    parser.add_argument('--start_epoch', default=0, type=int, metavar='N',
                        help='manual epoch number (useful on restarts)')
    parser.add_argument('--save_every_epoch', default=False, action='store_true', help='Flag to set a separate checkpoint file for each new epoch')
    parser.add_argument('--threads', type=int, default=6, help='Number of threads for each data loader to use')
    parser.add_argument('--cuda', default=True, action='store_true', help='If true, use GPU. Else use CPU.')
    ...
    dataset = config['global_params']['dataset']
    subsets = config['global_params']['subsets']
    evalset= config['global_params']['evalset']
    ...
    if opt.resume_path: # if already started training earlier and continuing
        if isfile(opt.resume_path):
            ...
            if config['global_params']['pooling'] in ['netvlad','patchnetvlad']:
        ...
        model = get_model(encoder, encoder_dim, config['global_params'], append_pca_layer=False)
        print(f"Pooling {config['global_params']['pooling']}")
        if config['global_params']['pooling'] in ['netvlad','patchnetvlad']:
            ...
            if opt.cluster_path:
                ...
                if dataset == "Cambridge":
                    train_dataset = CambridgeLandmarks(opt.dataset_root_dir, subset=subsets, mode='train', transform=input_transform())
                elif dataset == "MSLS":
                    train_dataset = MSLS(opt.dataset_root_dir, cities=subsets, mode='train', transform=input_transform(),
    ...
    if config['train']['optim'] == 'ADAM':
        optimizer = optim.Adam(filter(lambda par: par.requires_grad,
                                      ...
                                      weight_decay=float(config['train']['weightDecay']))  # , betas=(0,0.9))
    ...
    if dataset == "Cambridge":
        train_dataset = CambridgeLandmarks(opt.dataset_root_dir, subset=subsets, mode='train', nNeg=int(config['train']['nNeg']), transform=input_transform(),
                            transform_1d=input_transform_1d(), channels=channels)

        validation_dataset = CambridgeLandmarks(opt.dataset_root_dir, subset=evalset, mode='val',transform=input_transform(),
                            transform_1d=input_transform_1d(), channels=channels)
    elif dataset == "MSLS":
        exlude_panos_training = not config['train'].getboolean('includepanos')
        train_dataset = MSLS(opt.dataset_root_dir, cities=subsets, mode='train', nNeg=int(config['train']['nNeg']), transform=input_transform(),
                            transform_1d=input_transform_1d(), channels=channels,
                            cached_queries=int(config['train']['cached_queries']), cached_negatives=int(config['train']['cached_negatives']),
        ...
        validation_dataset = MSLS(opt.dataset_root_dir, cities=subsets, mode='val', transform=input_transform(),
                                transform_1d=input_transform_1d(), channels=channels,
                                cached_queries=int(config['train']['cached_queries']), cached_negatives=int(config['train']['cached_negatives']),
                                bs=int(config['train']['cachebatchsize']), threads=opt.threads,
    ...
    print('===> Training query set:', len(train_dataset.qImages))
    ...
    for epoch in trange(opt.start_epoch + 1, opt.nEpochs + 1, desc='Epoch number'.rjust(15), position=0):
        train_epoch(train_dataset, model, optimizer, criterion, encoder_dim, device, epoch, opt, config, writer, dataset)
        if scheduler is not None:
           scheduler.step(epoch)
        if (epoch % int(config['train']['evalevery'])) == 0:
            recalls = val(validation_dataset, model, encoder_dim, device, opt, config, writer,dataset, channels=channels, epoch_num=epoch,
                          write_tboard=True, pbar_position=1)
            is_best = recalls[K_param] > best_score
            if is_best:
                not_improved = 0
                best_score = recalls[K_param]
            else:
                not_improved += 1
